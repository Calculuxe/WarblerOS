        ...
        while (byte = scan()) {
            if (byte == 0x1c) {
                if (strlen(buff) > 0 && strcmp(buff, "exit") == 0) {
                    ...
                    outw(0x4004, 0x3400);
                    outw(0x600, 0x34);
                    printf("-Error 4: WarblerOS failed to shut down. WarblerOS can currently only shut down on Virtual Machines. Turn off your computer manually-");
                } else if (strlen(buff) > 0 && strcmp(buff, "about") == 0) {
                    printf("\n--About WarblerOS--");
                    printf("\n-WarblerOS Pre-Alpha Build 2609.04-");
                    ...
                    printf("\n--Devlog--");
                    printf("\n-Current Build: 2609.04-");
                    printf("\n-Fixed the bug with the backspace key-");
                    printf("\n-Added shutdown, currrently only has virtual machine support-");
                    printf("\n-Fixed the terminal cursor going everywhere-");
                    printf("\n-Added 'text' and 'bg' attribute to the 'color command'");
                    printf("\n-Added Error 3 and Error 4-");
                } else if (strlen(buff) > 0 && strcmp(buff, "bugs") == 0) {
                    printf("\n--Known Bugs For Build 2609.04--");
                    printf("\n-Arrow keys count as number keys-");
                    printf("\n-WarblerOS will not shut down on real hardwarre or unsupported hypervisors such as vmware-");
                    printf("/nYou will have to turn your computer manually.");
                ...
                } else if (strlen(buff) > 0 && strcmp(buff, "color text dark red") == 0) {
                    terminal_set_color(COLOR_RED, COLOR_BLACK);
                } else if (strlen(buff) > 0 && strcmp(buff, "color text purple") == 0) {
                    ...
                    terminal_initialize();
                    printf("\n--Choose a background color by typing: 'color text' and then the color of choice--");
                    printf("\n--Choose a text color by typing: 'color text' and then the color of choice--");
                    printf("\n-blue-");
                    printf("\n-green-");
                    printf("\n-red-");
                ...
                } else if (strlen(buff) > 0 && strcmp(buff, "color bg dark red") == 0) {
                    terminal_set_color(COLOR_WHITE, COLOR_RED);
                } else if (strlen(buff) > 0 && strcmp(buff, "color bg purple") == 0) {
